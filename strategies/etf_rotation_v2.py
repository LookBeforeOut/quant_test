...
class ETFRotationStrategyV2(Strategy):
    """增强版动量轮动策略"""

    def __init__(self, symbols: List[str], config: ETFRotationConfigV2):
        ...
        self._momentum_rank = pd.DataFrame()
        self._volatility_filter = pd.DataFrame()
    
    def _calculate_momentum_score(self, prices: pd.DataFrame) -> pd.Series:
        """三因子复合动量：
        1. 63日波动调整动量（主因子）
        2. 21日短期动量
        3. 50日均线趋势过滤
        ...
        returns = pd.DataFrame({
            f'ret_{self.config.momentum_window}': prices.pct_change(self.config.momentum_window),
            'ret_21': prices.pct_change(21),
            'ret_63': prices.pct_change(63)
        })
        trend_filter = (prices > prices.rolling(50).mean()).astype(int)
        vol = prices.pct_change().rolling(21).std()
        adj_returns = returns.div(vol + 1e-6, axis=0)
        momentum_score = (
            0.5 * adj_returns.ret_63 + 
            0.3 * adj_returns.ret_21 + 
            0.2 * trend_filter.mean(axis=1)
        )
        ...
        try:
            ...
            for symbol in self.symbols:
                try:
                    ...
                    # 计算回报率
                    total_return = self.data_processor.calculate_total_return(prices)
 
                    # 检查计算结果的有效性
                    if np.isnan(total_return):
                        logger.warning(f"Invalid indicator values for {symbol}, skipping...")
                        signals[symbol] = -np.inf                
                    else:
                        signals[symbol] = total_return
            ...
            logger.debug(f"Signals for symbols: {signals}")
